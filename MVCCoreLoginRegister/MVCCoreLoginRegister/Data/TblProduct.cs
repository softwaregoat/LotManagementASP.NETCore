using System;
using System.Collections.Generic;

namespace MVCCoreLoginRegister.Data
{
    public partial class TblProduct
    {
        public int ProduceId { get; set; }
        public string ProductName { get; set; }
        public string Des { get; set; }
        public string Col { get; set; }
        public string Tol { get; set; }
        public int? Cav { get; set; }
        public decimal? Cyc { get; set; }
        public decimal? Cph { get; set; }
        public decimal? Pps { get; set; }
        public bool? Rtq { get; set; }
        public bool? Fut { get; set; }
        public bool? Sts { get; set; }
        public bool? Ens { get; set; }
        public bool? Coc { get; set; }
        public bool? Coa { get; set; }
        public bool? Spc { get; set; }
        public bool? Por { get; set; }
        public string P1 { get; set; }
        public int? P1q { get; set; }
        public string P2 { get; set; }
        public int? P2q { get; set; }
        public string Mb { get; set; }
        public int? Mbq { get; set; }
        public string Sa { get; set; }
        public int? Saq { get; set; }
        public string Pa { get; set; }
        public int? Paq { get; set; }
        public string Ca { get; set; }
        public int? Caq { get; set; }
        public string Cl { get; set; }
        public int? Clq { get; set; }
        public string Ib { get; set; }
        public int? Ibq { get; set; }
        public string Dwg { get; set; }
        public string Pqis { get; set; }
        public string Ps { get; set; }
    }
}
